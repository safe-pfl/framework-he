...
class Client(FederatedBase):
    ...
    def generate_pubkey(self, vector_a: List[int]) -> List[int]:
        ...
        self.log.info(f"client id: {self.id} public key is:")
    ...
    def encrypt_parameters(self) -> Tuple[List[int], List[int]]:
        ...
        flat_weights = torch.cat([param.data.view(-1) for param in self.model.parameters()])
        ...
        flattened_weights, self.model_size = pad_to_power_of_2(flat_weights, self.rlwe.n, self.config.XMKCKKS_WEIGHT_DECIMALS)
    ...
    def compute_decryption_share(self, csum1, n, q) -> List[int]:
        ...
        decryption_share = list(decryption_share.poly.coeffs) #decryption_share is poly_t not poly_q
